% SETUP_MOISLT Initial configuration of MOISLT.
%   [DATA,PARAMS] = SETUP_MOISLT(X,Y,VARARGIN) sets the initial configuration 
%   of the MOISLT algorithm. X is a dataset of N-by-D size (N instances and D 
...
%   [data,params] = setup_moislt(X,Y,{10,10}); % Setup with: 
%                                              % - 10 individuals 
%                                              % - 10 generations 
%                                              % - All the other parameters are set to default values 
%   
%   See also MOISLT MINMAXNORM
    
% ------------------------------------------------------------------------
%   Cinvestav-IPN (Mexico)
%   SETUP_MOISLT Version 1.0 (Matlab R2022a)
%   June 2022
%   Copyright (c) 2022, Samuel Omar Tovias Alanis 
% ------------------------------------------------------------------------
    
function [data,params] = setup_moislt(X,Y,varargin)
% Default parameters 
...
params = {'np','gen','pc','pm','distance','p','hopt','dataset','numexp'}; 
    for i = 1:inputs
...
% Setup parameters 
params = setparams_moislt(X,Y,np,gen,pc,pm,distance,p,hopt,dataset,numexp);
% Check mutation probability value 
if strcmp(params.pm,'1/L'); params.pm = 1/params.chrlen; end 
end 
%***************************
% Setup parameters of MOISLT  
function params = setparams_moislt(X,Y,np,gen,pc,pm,distance,p,hopt,dataset,numexp)
% General information -------------------                          
params = struct();                      % Initialize the structure of the input parameters
params.algorithm = 'MOISLT';            % Name of the proposed algorithm 
params.metaheuristic = 'NSGA2';         % Multiobjective optimization algorithm  
...
% Multiobjective problem parameters ----- 
params.nobj = 2;                        % Number of objectives  
