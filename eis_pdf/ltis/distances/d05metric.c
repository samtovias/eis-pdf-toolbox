/*     
    C MEX Function.
    D05METRIC Yang unnormalized distance of order 0.5.  
        YD05 = D05METRIC(X1,X2) computes the Yang unnormalized distance between 
        two single vectors. X1 and X2 are single vectors of 1-by-D size. YD05 
        is the Yang unnormalized distance of order 0.5. 
        
        Compile MEX function: 
        --------------------
        mex -O d05metric.c -largeArrayDims; % Build an executable file for Matlab engine
        ...
        yd05 = d05metric(x1,x2);            % Yang unnormalized distance
        ...
        D05METRIC Version 1.0 (Matlab R2022a)
        June 2022
...
void dist_xy(double* x, double* y, double* dist, size_t rows, size_t cols){
    ...
    for (i=0; i<rows; i++) {
        ...
        dist[i] = pow(pow(t1,0.5)+pow(t2,0.5),2);
    }
}   
