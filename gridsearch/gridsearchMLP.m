% GRIDSEARCHMLP Grid search to fit the hyperparameters of an MLP classification model.
function [hn,grid] = gridsearchMLP(X,Y,params)
    if nargin < 3 
        K = 10;      
        name = '';
    else
    ...
    % Cross-validation and grid-search for MLP tuning
...
function grid = grid_search(X,Y,k,K,kf,hn,name)
    ...
    for i = 1:numel(hn)
        h = hn(i); 
        [model,exectime] = trainMLP(Xtr,Ytr,h);  % Training  
        Yp = predictMLP(model,Xvd);              % Validation 
...
function display_status(name,k,K,h,hn,exectime) 
    fprintf(['Grid search: MLP | Dataset: %s | Fold: %d/%d | Hidden nodes:' ...
